...
func (d *Reservation) GetReservationMetadata(adventureID string) (entity.ReservationMetadata, error) {
	metadata := entity.ReservationMetadata{AdventureID: adventureID}
...
func (d *Reservation) reserveIfAvailableCapacity(reservationUser entity.Reservation, update bool) error {
	...
	result := tx.Raw(`SELECT availability FROM reservation_capacities 
		...
		reservationUser.Quantity, reservationUser.AdventureID, reservationUser.Type).Scan(&availability)
	if result.Error != nil {
		tx.Rollback()
		return result.Error
	}
	...
	result = tx.Exec(`UPDATE reservation_capacities 
		...
		availability, reservationUser.AdventureID, reservationUser.Type)
	if result.Error != nil {
		tx.Rollback()
		return result.Error
	}
...
func (d *Reservation) CancelReservation(reservationUser entity.Reservation) error {
	...
	result := tx.Raw(`SELECT availability FROM reservation_capacities 
						WHERE adventure_id=? and 
							  type=? and deleted_at is null FOR UPDATE;`,
		reservationUser.AdventureID, reservationUser.Type).Scan(&availability)
	if result.Error != nil {
		tx.Rollback()
		return result.Error
	}
...
func (d *Reservation) AddMoreAvailability(reservationCapacity entity.ReservationCapacity) error {
	...
	result := tx.Raw(`SELECT availability FROM reservation_capacities 
						WHERE adventure_id=? and 
							  type=? and deleted_at is null FOR UPDATE;`,
		reservationCapacity.AdventureID, reservationCapacity.Type).Scan(&availability)
	if result.Error != nil {
		tx.Rollback()
		return result.Error
	}
	...
	result = tx.Exec(`UPDATE reservation_capacities 
						SET availability=? 
						WHERE adventure_id=? and type=? and deleted_at is null;`,
		reservationCapacity.Availability, reservationCapacity.AdventureID, reservationCapacity.Type)
	if result.Error != nil {
		tx.Rollback()
		return result.Error
	}
